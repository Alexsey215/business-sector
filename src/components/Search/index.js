...
const Search = () => {
    ...
    return (
        <>
            <Form onSubmit={searchHandler} action="" className={["d-flex mt-1 w-50", style.search__form]}>
                <input
                    className={style.search__form__input}
                    value={request}
                    onChange={e => setRequest(e.target.value)}
                    placeholder="Поиск"
                    type="text"/>
                <button className={style.search__form__btn} type="submit">
